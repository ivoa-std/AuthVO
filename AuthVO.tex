...
obtain it from the advertised discovery metadata.

\begin{description}
\item{\textbf{Token acquisition:}

Clients MUST use the Device Authorization Flow. This involves requesting a device code from the \verb|device_authorization_endpoint|, prompting the user to authenticate via a browser, and polling the \verb|token_endpoint| for completion. The request MUST include the \verb|client_id| assigned at registration:
...
If present, the \verb|allowed_origins| field (legacy VO mechanism) restricts the HTTP
origins at which the token may be used. Clients MUST verify both the scope and
the origin of usage.
