...
but the standard \verb|Bearer| challenge provides no information about how to
...
carries a pointer to a Protected Resource Metadata document (\rfc{9728}) from
which the client can discover the authorization server, verify that it is
entitled to issue tokens for this resource, and obtain a token whose audience
...
VO services that require OAuth~2.0 (\rfc{6749}) access tokens use the standard
...
discover the authorization server and how to obtain a token that is safe to
present to this particular resource, by combining the \verb|Bearer| scheme with
the Protected Resource Metadata mechanism of \rfc{9728}.

The scheme supports headless clients (e.g.\ command-line tools or scripts)
through the Device Authorization Grant (\rfc{8628}), token expiry and renewal
through refresh tokens, and zero-touch onboarding through Dynamic Client
Registration (\rfc{7591}); it integrates with standard OAuth2/OIDC
infrastructures.
...
\verb|resource_metadata| parameter (\rfc{9728}) pointing at a \emph{Protected
Resource Metadata} (PRM) document: a small JSON document, served from the
authorization server's own origin, that names the authorization server(s)
entitled to issue tokens for this resource. This indirection is deliberate ---
it is what lets the client confirm that the resource and the issuer genuinely
belong together \emph{before} parting with any credentials or token
...
\begin{description}
  ...
  \begin{itemize}
    \item \verb|resource_metadata| (required) --- the URL of the Protected
          Resource Metadata document (\rfc{9728}) describing this resource.
          It is an \emph{untrusted} pointer until the checks of
          Section~\ref{sec:ivoa-bearer-flow} have passed, and MUST be served
          over HTTPS.
    \item \verb|error| (optional) --- an OAuth~2.0 error code, e.g.\
          \verb|invalid_request|, \verb|invalid_token| or \verb|expired_token|.
...
When a protected resource is accessed without a valid token, the service
...
WWW-Authenticate: Bearer
  resource_metadata=
    "https://iam.example.org/.well-known/oauth-protected-resource",
  error="invalid_token",
  error_description="Missing or invalid access token"
\end{verbatim}

The \verb|resource_metadata| URL MUST resolve to a Protected Resource Metadata
document conforming to \rfc{9728}. Its key members are:

\begin{itemize}
  \item \verb|resource| --- the canonical identifier of the protected resource.
        The token's audience (\verb|aud|) is bound to this value, and every
        protected URL it governs MUST be at or below it in the URL path.
  \item \verb|authorization_servers| --- one or more issuer URLs entitled to
        mint tokens for this resource. The client trusts an entry only after
        the domain-proxy check of Section~\ref{sec:ivoa-bearer-flow}.
...
Authorization Server Metadata (\rfc{8414}, or OpenID Connect Discovery), which
provides the endpoints the flow needs:
...
Error codes carried in the \verb|error| parameter are those standardised by
OAuth~2.0; a representative subset is listed in Table~\ref{table:oAuthCodes}.
\begin{table}[]
\centering
\begin{tabular}{ll}
...
correct authorization server; (b) prove that the server and the resource belong
together \emph{before} parting with any credentials; and (c) obtain a token
bound to the target resource, so that the token is useless if leaked to a third
party.

The central threat addressed is the ``evil service'': a malicious or
compromised resource that answers a request with a 401 pointing at
attacker-controlled metadata, hoping the client will obtain a token from a
legitimate issuer and then present it to the attacker. The domain-proxy check
(Step~3) and audience binding (Step~6) are what close this hole.
...
\begin{verbatim}
 Client                 Resource server          Authz server + PRM
 (headless)             data.example.org         iam.example.org
   ...
   |<-------------------------|                          |
   |                                                     |
   | 2 GET <resource_metadata>   (PRM "sidecar")         |
   |---------------------------------------------------->|
   |   { resource, authorization_servers[], scopes }     |
   |<----------------------------------------------------|
   |                                                     |
   | 3 DOMAIN-PROXY CHECK (local, MUST pass):            |
   |     (a) origin(PRM) == origin(issuer)               |
   |     (b) requested URL covered by "resource"         |
   |                                                     |
   | 4 GET <issuer> AS metadata (RFC 8414); verify iss   |
   |---------------------------------------------------->|
   |   { token_endpoint, device_authorization_endpoint } |
   |<----------------------------------------------------|
   |                                                     |
   | 5 (if needed) register -> client_id (RFC 7591)      |
   |<--------------------------------------------------->|
   |                                                     |
   | 6 device grant (RFC 8628); user approves in browser |
   |   poll token_endpoint, resource=<resource>          |
   |   -> access_token bound to aud=<resource>           |
   |<--------------------------------------------------->|
   |                          |                          |
   | 7 GET /vo-resource, Authorization: Bearer <token>   |
   |------------------------->| verify sig/iss/aud/      |
...
\begin{verbatim}
GET https://iam.example.org/.well-known/oauth-protected-resource
\end{verbatim}
...
The client MUST retrieve this document over HTTPS and reads from it the
\verb|resource| identifier and the \verb|authorization_servers| list. Both
remain \emph{untrusted} until Step~3 succeeds.

\subsubsection{Step 3: Domain-proxy check (MANDATORY)}

This is the security core of the scheme. Before trusting the issuer named in the
PRM, the client MUST verify \emph{two} gates, and MUST abort the flow if either
fails:

\begin{description}
\item[Gate (a) --- same origin:] the origin of the \verb|resource_metadata| URL
      (from which the PRM was fetched) MUST equal the origin of the issuer named
      in \verb|authorization_servers|. \emph{Origin} is the normalised triple of
      scheme, host and port (\rfc{9110}, section 4.3.1). A forged descriptor on
      an attacker's host cannot satisfy this: only a document served from the
      issuer's own origin can vouch for that issuer.
\item[Gate (b) --- resource coverage:] the URL the client originally requested
      MUST be covered by the \verb|resource| value, i.e.\ be equal to it or
      below it at a path-segment boundary. This ensures the descriptor is
      authoritative for the very resource being accessed, and not merely a valid
      descriptor for some unrelated resource.
\end{description}

Co-locating the authorization server and the PRM document on a single origin is
therefore a deployment requirement of this scheme. The \verb|resource| id that
matched gate (b) is the audience the client will request in Step~6.

\subsubsection{Step 4: Retrieve and validate Authorization Server metadata}

Having established trust in the issuer, the client retrieves its metadata
...
The client MUST retrieve the document over HTTPS, MUST verify that its
\verb|issuer| value equals the issuer named in the PRM, and extracts the
endpoints it needs. If the authorization server later returns an issuer
identifier in token responses (\rfc{9207}), the client MUST verify that it
matches as well.

\textit{Note: \rfc{9207} (issuer identification in token responses) is optional,
as it is not universally implemented; if present it MUST be validated.}

\subsubsection{Step 5: Dynamic Client Registration (if needed)}
...
Step~6 rather than relying on a subsequent exchange.

\subsubsection{Step 6: Obtain an audience-bound token}

The client starts the Device Authorization Grant (\rfc{8628}):

\begin{verbatim}
POST https://iam.example.org/devicecode
client_id=s6BhdRkqt3&scope=openid read
\end{verbatim}
...
\begin{verbatim}
To authorize access to https://data.example.org, open
...
\verb|authorization_pending| and \verb|slow_down| signals, and MUST request that
the token be bound to the resource matched in Step~3. Two mechanisms are
defined:

\begin{itemize}
  \item the resource indicator \verb|resource| (or \verb|aud|) parameter
        (\rfc{8707}) on the device-grant token request --- the RECOMMENDED
        approach, since it works for public, self-registered clients; or
  \item a subsequent OAuth~2.0 Token Exchange (\rfc{8693}) that narrows the
        audience of an already-issued token.
\end{itemize}
...
On success the response carries a token whose audience is the target resource:
...
\textit{Note: \rfc{8707} (Resource Indicators) and \rfc{8693} (Token Exchange)
may not be universally implemented; where neither is available the token cannot
be audience-restricted, and the client SHOULD treat it as usable only with the
service that issued the challenge.}

\subsubsection{Step 7: Access the protected resource with the token}
...
\begin{itemize}
\item if the token is a JWT, verify its signature against the issuer's
      key set (\rfc{9068});
\item verify the issuer (\verb|iss|) matches the expected authorization server;
\item verify the audience (\verb|aud|) equals this resource's own identifier;
...
invalid or expired token, or 403 for an authenticated request that lacks the
required scope. On success it returns the protected resource.
...
When the original token was audience-bound, the refreshed token MUST be
requested with the same audience. Refresh tokens MUST be stored securely, as
they grant persistent access.

\subsubsection{Security Considerations}

\begin{itemize}
\item Clients MUST NOT trust the \verb|resource_metadata| pointer, the PRM
      document, or the issuer it names until the domain-proxy check (Step~3) has
      passed; a failed check MUST abort the flow.
\item Services MUST return only the \verb|resource_metadata| pointer in the
      challenge, and MUST serve the PRM document from the same origin as the
      authorization server it names.
\item Clients SHOULD obtain audience-bound tokens (Step~6) and MUST NOT reuse a
      token across unrelated services: a token bound to one resource MUST NOT be
      presented to another.
\item Services MUST validate token integrity, issuer, audience, scope and
      expiry (Step~7) before releasing any data.
